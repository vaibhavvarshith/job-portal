    ...
    <>
        ...
        <div className="applications-approval-container">
            <div className="applications-approval-toolbar">
                ...
                <div className="skeleton-box" style={{ height: '40px', width: '150px', marginRight: '1rem' }}></div>
                <div className="skeleton-box" style={{ height: '40px', width: '150px' }}></div>
            </div>
            <div className="applications-approval-table-container skeleton-box" style={{ padding: '20px', marginTop: '1rem' }}>
...
function ApplicationsApprovalStandalonePage() {
    ...
    const uniqueJobTypes = useMemo(() => ['All', ...new Set(applications.map(app => app.jobId?.jobType).filter(Boolean))], [applications]);
    const uniqueStatuses = useMemo(() => ['All', ...new Set(applications.map(app => app.status))], [applications]);
            ...
            <div className="recruiter-page-main-content">
                <header className="recruiter-page-main-header">
                    <div className="recruiter-page-header-content">
                        <h1 className="recruiter-page-header-title">Applications Approval</h1>
                    ...
                    {loading ? <ApplicationsSkeleton /> : (
                        <div className="applications-approval-container">
                            <div className="applications-approval-toolbar">
                                ...
                                />
                                <select value={filterJobType} onChange={(e) => { setFilterJobType(e.target.value); setCurrentPage(1); }} className="applications-approval-filter-select">
                                    {uniqueJobTypes.map(type => (
                                        <option key={type} value={type}>{type === 'All' ? 'All Types' : type}</option>
                                    ))}
                                </select>
                                <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value); setCurrentPage(1); }} className="applications-approval-filter-select">
                            ...
                            {paginatedApplications.length > 0 ? (
                                    ...
                                    <table className="applications-approval-table">
                                        ...
                                        <tbody>
                                            {paginatedApplications.map(app => (
                                                <tr key={app._id}>
                                                    <td>
                                                        <Link to={`/student/profile/${app.studentId?._id}`} className="student-profile-link" title="View Student Profile">
                                                    ...
                                                    <td>
                                                        <span className={`app-status-badge status-${getStatusClass(app.status)}`}>
                                                            {app.status}
                                                        </span>
                                                    </td>
                                                    <td className="document-links">
                                                        <a href={'#'} target="_blank" rel="noopener noreferrer" title="View Resume">
                                                            <i className="fas fa-file-pdf"></i> Resume
                                                        </a>
                                                    </td>
                                                    <td className="application-approval-actions-cell">
                                                        {app.status === 'New' &&
                                                            <button onClick={() => handleAction(app._id, 'Viewed')} className="action-btn-app-approval mark-viewed" title="Mark as Viewed"><i className="fas fa-eye"></i></button>
                                                        }
                                                        {(app.status === 'New' || app.status === 'Viewed') &&
                                                            <button onClick={() => handleAction(app._id, 'Shortlisted')} className="action-btn-app-approval shortlist" title="Shortlist"><i className="fas fa-star"></i></button>
                                                        }
                                                        {app.status === 'Shortlisted' &&
                                                            <button onClick={() => handleAction(app._id, 'Interview Scheduled')} className="action-btn-app-approval schedule" title="Schedule Interview"><i className="fas fa-calendar-plus"></i></button>
                                                        }
                                                        {(app.status !== 'Rejected') &&
                                                            <button onClick={() => handleAction(app._id, 'Rejected')} className="action-btn-app-approval reject" title="Reject"><i className="fas fa-times-circle"></i></button>
                                                        }
                                                    </td>
                                                </tr>
                                            ))}
                                ...
                                <div className="no-applications-to-review">
                                    ...
                                    <p>No applications found matching your criteria or awaiting review.</p>
                                </div>
                            )}
