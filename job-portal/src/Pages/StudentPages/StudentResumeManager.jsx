...
function ResumeManagerStandalonePage() {
    ...
    const [resumeScoreResult, setResumeScoreResult] = useState(null);     const scoreFileInputRef = useRef(null);
    const [isCheckingScore, setIsCheckingScore] = useState(false);
        ...
        const fetchResumesAndProfile = async () => {
            ...
            try {
                                const resumesRes = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/student/resumes`, {                     headers: { 'Authorization': `Bearer ${token}` }
                });
                ...
                if (profileRes.ok) {
                    ...
                    setStudentData(profileData);
                        ...
                        personalDetails: {
                            ...
                            email: profileData.user?.email || '',                             phone: profileData.phone || '',
        ...
        if (file) {
            if (file.size > 5 * 1024 * 1024) {                 toast.error("File size exceeds 5MB limit.");
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/student/resumes/upload`, {                 method: 'POST',
                headers: {
            ...
            toast.success(result.message);
                ...
                const updatedResumes = prevResumes.map(r => ({ ...r, isDefault: false }));                 return [...updatedResumes, result.resume];             });
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/student/resumes/${resumeId}`, {                 method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` },
            });
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/student/resumes/${resumeId}/set-default`, {                 method: 'PATCH',
                headers: { 'Authorization': `Bearer ${token}` },
            });
    ...
    const handleAiSimpleInputChange = (section, event) => {         const { name, value } = event.target;
        setAiResumeData(prev => ({
            ...prev,
    ...
    const addAiEntry = (section, initialEntry) => {
        setAiResumeData(prev => ({
            ...prev,
            [section]: [...prev[section], { ...initialEntry, _id: Date.now().toString() }],         }));
    };
    ...
    const handleGenerateAiResume = async (event) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/student/resumes/ai-build`, {                 method: 'POST',
                headers: {
    ...
    const handleCheckResumeScore = async (event) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/student/resumes/check-score`, {                 method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
                body: formData,
