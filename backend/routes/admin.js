...
router.get("/recruiter-approvals", auth, adminAuth, async (req, res) => {
  try {
    ...
    if (searchTerm) {
      ...
      requestsWithCompanyInfo = requestsWithCompanyInfo.filter(
        (req) =>
    ...
    res
      .status(500)
      .json({ message: "Server error fetching recruiter approvals." });
  }
});
...
router.get("/industries", auth, adminAuth, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ industries: industries.filter((i) => i && i.trim() !== "") });
  } catch (error) {
 ...
 */
router.patch(
  "/recruiter-approvals/:id/approve",
  auth,
  adminAuth,
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(404)
          .json({
            message: "Recruiter approval request not found or not pending.",
          });
      }
      ...
      res
        .status(200)
        .json({ message: "Recruiter approved successfully!", user });
    } catch (error) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Recruiter rejected successfully!", user });
    } catch (error) {
...
router.get("/analytics/dashboard-data", auth, adminAuth, async (req, res) => {
  try {
    ...
    const userGrowthTotal = userGrowthStudents.map(
      (s, i) => s + userGrowthRecruiters[i]
    );
      ...
      datasets: [
        {
          ...
          data: internshipsPosted,
          backgroundColor: "#6366F1",
          borderRadius: 4,
        },
        {
          label: "Full-time Jobs Posted",
          data: fullTimeJobsPosted,
    ...
    const shortlistedApps = await Application.countDocuments({
      status: "Shortlisted",
    });
    const interviewedApps = await Application.countDocuments({
      ...
      datasets: [
        {
          ...
          borderColor: ["#3B82F6", "#10B981", "#F59E0B", "#EC4899"],
          borderWidth: 1,
        },
      ],
    };
...
router.post("/analytics/generate-report", auth, adminAuth, async (req, res) => {
  ...
  let reportData = [];
  let reportTitle = `Report: ${reportType.replace(
    /_/g,
    " "
  )} for ${dateRange.replace(/_/g, " ")}`;
  ...
  try {
    switch (reportType) {
      ...
      case "application_stats":
        reportData = await Application.find({
          createdAt: { $gte: startDate, $lte: endDate },
        })
