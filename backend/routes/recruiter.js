...
router.get('/dashboard-stats', auth, async (req, res) => {
    ...
    try {
        ...
        const activeInternships = await Internship.countDocuments({ postedBy: recruiterId });
        const totalApplications = await Application.countDocuments({ recruiterId: recruiterId });
        const shortlisted = await Application.countDocuments({ recruiterId: recruiterId, status: 'Shortlisted' });
...
router.get('/recent-applications', auth, async (req, res) => {
    ...
    try {
        const recentApplications = await Application.find({ recruiterId: req.user.id })
